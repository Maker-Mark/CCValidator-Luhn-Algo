...
luhn = (cardNumStr) => { // cardNumStr as a string, assumes digits are there
    let num, flag = false; // flag to hit every other number
    //Turn the string into a char array, reverse it so we can use the reduce function.
    return (cardNumStr.split('').reverse().reduce(
        function (sum = 0, curr) {
            ...
            return (sum + ((flag = !flag) ? num : [0, 2, 4, 6, 8, 1, 3, 5, 7, 9][num]));
        }, 0 //Initialize the first value to zero(defaults to the first element, normally)
    ) % 10 == 0); // Return if the remainder is zero
}

validateNum = (cardNumStr) => {
    cardNumStr += ""
    amex = () => ((cardNumStr.substring(0, 2) == "34") || (cardNumStr.substring(0, 2) == "37")) ? "Amex" : null;
